...
set_family_t *
sf_ind_contain(set_family_t *A, int *row_indices)
{
    int cnt;
    set **A1;
    set_family_t *R;
...
set_family_t *
sf_union(set_family_t *A, set_family_t *B)
{
    int cnt;
    set_family_t *R;
    set **A1 = sf_list(A), **B1 = sf_list(B), **E1;

    E1 = ALLOC(set *, MAX(A->count, B->count) + 1);
...
set_family_t *
dist_merge(set_family_t *A, set *mask)
{
    set **A1;
    int cnt;
    set_family_t *R;
...
set_family_t *
d1merge(set_family_t *A, int var)
{
...
int
rm2_equal(set **A1, set **B1, set **E1, int (*compare)(set **, set **))
{
    set **pda = A1, **pdb = B1, **pde = E1;
...
int
rm2_contain(set **A1, set **B1)
{
    set **pa, **pb, *a, *b, **pdest = A1;
...
set **
sf_sort(set_family_t *A, int (*compare)(set **, set **))
{
    set *p, *last, **pdest, **A1;

    // Create a single array pointing to each cube of A
    pdest = A1 = ALLOC(set *, A->count + 1);
...
set_family_t *
sf_unlist(set **A1, int totcnt, int size)
{
    set *pr, *p, **pa;
    set_family_t *R = sf_new(totcnt, size);
...
set_family_t *
sf_ind_unlist(set **A1, int totcnt, int size, int *row_indices, set *pfirst)
{
    set *pr, *p, **pa;
...
set_family_t *
sf_merge(set **A1, set **B1, set **E1, int totcnt, int size)
{
    set *pr, *ps, **pmin, **pmid, **pmax;
    set_family_t *R;
    set **temp[3], **swap;
    int i, j, n;
