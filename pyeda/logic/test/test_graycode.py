...
from pyeda.boolalg.vexpr import bitvec, uint2bv
from pyeda.logic.graycode import bin2gray, gray2bin

def test_bin2gray():
    B = bitvec('B', 4)
    G = bin2gray(B)
    gnums = [G.vrestrict({B: uint2bv(i, 4)}).to_uint() for i in range(16)]
...
def test_gray2bin():
    ...
    bnums = [B.vrestrict({G: uint2bv(i, 4)}).to_uint() for i in gnums]
