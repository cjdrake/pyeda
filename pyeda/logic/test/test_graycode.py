...
from pyeda.boolalg.bfarray import exprvars, uint2array
from pyeda.logic.graycode import bin2gray, gray2bin

def test_bin2gray():
    B = exprvars('B', 4)
    G = bin2gray(B)
    gnums = [G.vrestrict({B: uint2array(i, 4)}).to_uint() for i in range(16)]
...
def test_gray2bin():
    ...
    bnums = [B.vrestrict({G: uint2array(i, 4)}).to_uint() for i in gnums]
