...
def test_exactlyn():
    ...
    """
    is_sat = lambda x: True if x.satisfy_one() else False
    assert is_sat(ExactlyN(0, 0, 0, k=2)) == False
    assert is_sat(ExactlyN(0, 0, 1, k=2)) == False
    assert is_sat(ExactlyN(0, 1, 0, k=2)) == False
    assert is_sat(ExactlyN(0, 1, 1, k=2)) == True
    assert is_sat(ExactlyN(1, 0, 0, k=2)) == False
    assert is_sat(ExactlyN(1, 0, 1, k=2)) == True
    assert is_sat(ExactlyN(1, 1, 0, k=2)) == True
    assert is_sat(ExactlyN(1, 1, 1, k=2)) == False

def test_atleastn():
    """ see remark above (in test_exactlyn)
    """
    is_sat = lambda x: True if x.satisfy_one() else False
    assert is_sat(AtLeastN(0, 0, 0, k=2)) == False
    assert is_sat(AtLeastN(0, 0, 1, k=2)) == False
    assert is_sat(AtLeastN(0, 1, 0, k=2)) == False
    assert is_sat(AtLeastN(0, 1, 1, k=2)) == True
    assert is_sat(AtLeastN(1, 0, 0, k=2)) == False
    assert is_sat(AtLeastN(1, 0, 1, k=2)) == True
    assert is_sat(AtLeastN(1, 1, 0, k=2)) == True
    assert is_sat(AtLeastN(1, 1, 1, k=2)) == True

def test_atmost_n():
    """ see remark above (in test_exactlyn)
    """
    is_sat = lambda x: True if x.satisfy_one() else False
    assert is_sat(AtMostN(0, 0, 0, k=2)) == True
    assert is_sat(AtMostN(0, 0, 1, k=2)) == True
    assert is_sat(AtMostN(0, 1, 0, k=2)) == True
    assert is_sat(AtMostN(0, 1, 1, k=2)) == True
    assert is_sat(AtMostN(1, 0, 0, k=2)) == True
    assert is_sat(AtMostN(1, 0, 1, k=2)) == True
    assert is_sat(AtMostN(1, 1, 0, k=2)) == True
    assert is_sat(AtMostN(1, 1, 1, k=2)) == False
