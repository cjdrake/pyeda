 ...
 * Routines:
 * set_family_t *ess_test_and_reduction():
 ...
 *     inessential.
 * set_family_t *aux_ess_test_and_reduction():
...
set_family_t *COVER; /* A global bag to collect the signature cubes in the cover
...
set *c;
{
	...
	minterms = sf_new(1, cube.size);
	d_minterm = set_new(cube.size);
