...
set_family_t *
expand(set_family_t *F, set_family_t *R, bool nonsparse)
/* expand non-sparse variables only */
{
    set *last, *p;
    set *RAISE, *FREESET, *INIT_LOWER, *SUPER_CUBE, *OVEREXPANDED_CUBE;
    int var, num_covered;
    bool change;
    ...
    INIT_LOWER = set_new(CUBE.size);
    SUPER_CUBE = set_new(CUBE.size);
    OVEREXPANDED_CUBE = set_new(CUBE.size);
...
void
expand1(set_family_t *BB, set_family_t *CC, set *RAISE, set *FREESET, set *OVEREXPANDED_CUBE, set *SUPER_CUBE,
        set *INIT_LOWER, int *num_covered, set *c)
...
void
essen_parts(set_family_t *BB, set_family_t *CC, set *RAISE, set *FREESET)
{
    set *p, *r = RAISE;
    set *lastp, *xlower = CUBE.temp[0];
    int dist;
...
void
essen_raising(set_family_t *BB, set *RAISE, set *FREESET)
{
    set *last, *p, *xraise = CUBE.temp[0];
...
{
    set *p, *r = set_or(CUBE.temp[0], RAISE, FREESET);
    set *last;
...
int
most_frequent(set_family_t *CC, set *FREESET)
{
...
void
setup_BB_CC(set_family_t *BB, set_family_t *CC)
{
    set *p, *last;
...
void
select_feasible(set_family_t *BB, set_family_t *CC, set *RAISE, set *FREESET, set *SUPER_CUBE, int *num_covered)
{
    set *p, *last, *bestfeas, **feas;
    int i, j;
    set **feas_new_lower;
    int bestcount, bestsize, count, size, numfeas, lastfeas;
    set_family_t *new_lower;

    // Start out with all cubes covered by the over-expanded cube as
    // the "possibly" feasibly-covered cubes (pfcc)
    feas = ALLOC(set *, CC->active_count);
    numfeas = 0;
    foreach_active_set(CC, last, p)
        feas[numfeas++] = p;

    // Setup extra cubes to record parts forced low after a covering
    feas_new_lower = ALLOC(set *, CC->active_count);
...
bool
feasibly_covered(set_family_t *BB, set *c, set *RAISE, set *new_lower)
{
    set *p, *r = set_or(CUBE.temp[0], RAISE, c);
    int dist;
    set *lastp;
...
void
mincov(set_family_t *BB, set *RAISE, set *FREESET)
{
    int expansion, nset, var, dist;
    set_family_t *B;
    set *xraise=CUBE.temp[0], *xlower, *p, *last, *plower;
    ...
    essen_parts(BB, /*CC*/ (set_family_t *) NULL, RAISE, FREESET);
#else
    ...
    set_copy(FREESET, CUBE.emptyset);   // free set is empty
...
heuristic_mincov:
    ...
    set_insert(RAISE, most_frequent(/*CC*/ (set_family_t *) NULL, FREESET));
    set_diff(FREESET, FREESET, RAISE);
    essen_parts(BB, /*CC*/ (set_family_t *) NULL, RAISE, FREESET);
...
set_family_t *
find_all_primes(set_family_t *BB, set *RAISE, set *FREESET)
{
    set *last, *p, *plower;
    set_family_t *B, *B1;
...
{
    set *last, *p, *RAISE, *FREESET;
    set_family_t *Fall_primes, *B1;
    ...
    foreach_set(F, last, p) {
        if (TESTP(p, PRIME)) {
            ...
            set_copy(RAISE, p);
            set_diff(FREESET, CUBE.fullset, RAISE);
            setup_BB_CC(R, /* CC */ (set_family_t *) NULL);
            essen_parts(R, /* CC */ (set_family_t *) NULL, RAISE, FREESET);
